...
Massive neutrinos in the cosmic neutrino background also affect the growth of large scale structure. 
Massive neutrinos behave as light thermal relics, suppressing clustering below their thermal
free-streaming length: for reviews of this effect see \cite{Lesgourgues_2006, Wong_2011, Lesgourgues_2014}. 
...
measurements~\cite{Beutler_2011, Anderson_2014, Ross_2015}. Ref.~\cite{Palanque_2015} found a tighter constraint of $M_\nu<0.15$~eV by combining Ly$\alpha$ forest data from the Sloan Digital Sky Survey (SDSS) with Planck. However, recent weak lensing data from the Dark Energy Survey combined with Planck weakens the upper limit to $0.29$ eV \citep{DES_2017}, and the most recent galaxy power spectrum measurements from SDSS show a slight preference for a non-zero neutrino mass of $M_\nu = 0.3$ eV \citep{Beutler_2014}.
...
Furthermore, current and future experiments achieve their statistical power from small scales where structure formation is in the non-linear regime \citep[e.g.~][]{Troxel_2017, HSC_2017}. On large scales structure growth can be modelled analytically (e.g.,~\cite{Bond_1980,Zeldovich_1980,Ma_1994}). However, modelling non-linear effects on small scales requires $N$-body particle simulations.

%%%%%EDIT
The \texttt{Halofit} formalism~\cite{smith2003,takahashi2012,Bird2012,Mead2015,Mead2016,Casarini2009,Casarini2016}, based on the ``halo model''~\cite{PeacockSmith2000,Seljak2000} and calibrated against numerical simulations, has been widely used to provide a fitting function for the matter power spectrum in the nonlinear regime. Alternatively, a series of papers dubbed the ``Coyote Universe'' developed a prediction scheme called the ``emulator'', based on interpolating the measured $P(k)$ from a large number of simulations with various resolutions and cosmological models.  The emulator reached percent-level accuracy out to $k\approx 10$/Mpc and $z=4$ for $w$CDM cosmologies~\cite{heitmann2010i,heitmann2009ii,heitmann2010iii,heitmann2014e}. However, among all these efforts, only \cite{Bird2012} included massive neutrino {\red particles} in their simulation, and with only 3 different neutrino masses (0.15, 0.3 and 0.6~eV). Our work has a much denser sampling of neutrino masses, and hence is suitable to study the nonlinear effects of massive neutrinos as well as their degeneracy with other cosmological parameters.
...
and thus the only way to achieve this is
through cosmological simulations. These simulations should include a massive neutrino model if the scales probed overlap with the free-streaming scale of the neutrino. For example, massive
neutrinos are included as a free parameter in the ``Coyote'' matter power simulation suite \citep{Lawrence_2017}. Thus we require methods for incorporating massive neutrinos into cosmological structure simulations.

These methods should be both computationally inexpensive and accurate. If the simulation methods used are insufficiently accurate, experiments will measure different values for the neutrino mass with the same level of cosmic structure.
...
In this paper we present improvements to our earlier neutrino simulation method from \cite{AHB}. This method is computationally efficient and accurately describes the effect of massive neutrinos
on the growth of cold dark matter. However, our implementation was difficult to use, being tied to a non-public version of the simulation code Gadget-3 \cite{Springel_2005}.
Furthermore, while our method described the cold dark matter and total matter power spectrum very accurately, it was not able to model the neutrino power spectrum on small scales,
and was limited to $M_\nu < 1 $ eV. These limitations are not restrictive: observations require $M_\nu \ll 1$ eV and the neutrino power spectrum is not currently directly
observable (but see \citep{Ptolemy}). However, in this paper we present a modification of our method which is portable to many different structure simulation codes, including the public version of Gadget-2,
and removes both these restrictions. Our model is a hybrid approach, combining our earlier analytic framework for low neutrino masses and early times with a neutrino particle implementation for
heavier neutrinos at late times. This allows for the first time a single simulation code to produce a well-converged neutrino simulation, at any neutrino mass, which includes
both early-time relativistic effects and late-time non-linear growth in the neutrino sector.
