...
Figure~\ref{fig:hybparticle} compares the neutrino power spectrum from the particle and hybrid methods in more detail. There are two interesting features: a $5\%$ deficit of power in the particle simulations at $k \approx 1$ $h$/Mpc, and a $5\%$ increase in power at $k \approx 0.2$ $h$/Mpc. The origins of these features are unclear, but are probably errors in the particle simulation induced by neutrino shot noise \yah{Saying such things is risky with this referee. How about leaving it to strictly what we know, i.e. that the origin is unclear?}. Note that the feature at $k \approx 1$ $h$/Mpc is less pronounced in the higher-resolution particle simulation \yah{But the feature at 0.2 is more pronounced... Let's not just talk about the good sides :) I suggest removing this comment altogether.}. We also show a hybrid simulation (\texttt{HYBRID-v5000}) where all neutrino mass density is in particles. This simulation evolves like a particle simulation after the neutrino switch-on time of $z=1$, but is identical to a linear response simulation before that. \texttt{HYBRID-v5000} reproduces the reduced power at $k\approx 1$ $h$/Mpc, suggesting that this is an effect of low-redshift shot noise from fast-moving neutrino particles. The \texttt{HYBRID} simulation does not suffer from this error, because it only includes slow neutrinos as particles. \yah{Are we happy with the large deficit that this one has at low-k? It is difficult to explain, and we may get dinged on that too... Again, we are only discussing the positive things, not the equally large negatives. I would suggest just showing all the plots, saying that the differences between all curves are less that 5 percent, that we don't fully understand these differences, but they make a minor effect on the observables.}
