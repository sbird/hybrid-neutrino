...
\earr
Note that the initial conditions piece is negligibly small in most situations.
...
This approximation is certainly accurate on linear scales, where modes are decoupled. AHB13 argued further that it does not affect the asymptotic small-scale behaviour, where early-time contributions to Eq.~\eqref{eq:delta-phi} are suppressed. Thus all that is needed is the present-day phase information and, as a consequence, this approximation ought to be reasonably accurate on all scales.
We can further check it's consistency using a particle neutrino simulation. Eq.~\eqref{eq:phases} implies that the cross-correlation between neutrinos and CDM is unity. Figure \ref{fig:cross-corr} shows the cross-correlation coefficient between neutrino particles and CDM in a particle simulation and in our new hybrid method, demonstrating that it is indeed close to unity as long as the neutrino power spectrum is not dominated by particle shot noise. \yah{I am not sure this plot is the best way to validate this approximation (but perhaps it can serve another purpose). First, Eq.~\eqref{eq:phases} \emph{implies}, but is \emph{not equivalent} to a unit cross-correlation coefficient. Therefore, showing that this coefficient is unity is a consistency check at best, but does not fully validate the approximation. Second, the approximation only applies to LRA neutrinos, \emph{not} to the slow neutrinos, so showing their correlation coefficient (green line) is not relevant for this specific point (unless you wanted to make another point?). Since the red curve contains the slow neutrinos as well, which we do not expect to have a unit correlation, it is not unexpected that it falls below unity (and it is hard to quantify by how much we expect it to do so). What would have been a good check is to show the cross-correlation of fast neutrinos only (from the particle sims), but this is probably not easily extracted. All in all, I think that the agreement with Banerjee et al. is sufficient to validate both layers of approximation, linear response \emph{and} phase approximation.}
\spb{I agree that this plot is mostly a consistency check, but I think it is a valuable one that many people will ask for. I might be able to extract just the initially fast neutrinos from the particle simulation using the IDs and initial velocities, as well, so I'll try that. Showing the full particle distribution is still relevant for the LRA only simulations though.}
...
In AHB13, the linear-response approximation was used for the \emph{entire} phase-space of neutrinos. Yet, this approximation should eventually break down below some critical velocity $u_{\rm crit}$, as the behaviour of neutrinos approaches that of the CDM. In this section we estimate $u_{\rm crit}$. Whenever required, we use the non-linear matter power spectrum estimated by \textsc{class} \citep{Lesgourgues_11, Blas_11} with the \textsc{halofit} approximation \citep{Smith_2003}.
...
\eeq
Therefore, by treating \emph{all} neutrinos with the linear-response approximation at $z \geq 1$, we underestimate the clustering of at most $\sim 0.1\%$ of the neutrinos, given current upper bounds to their masses. The from inaccurately following this small fraction of neutrino density should also be small, so we deem it safe to use the method of AHB13 (the LRA applied to \emph{all} neutrinos) for $z \geq 1$, i.e.~to set $z_\nu = 1$. \spb{Softened this sentence a bit: if these $0.1\%$ of neutrinos cluster $100$ times more than the others, they can still have a $10\%$ effect on the neutrino power spectrum, so it is not a priori clear that the error is small.}
...
\begin{figure*}
\includegraphics[trim={1.cm 0 1.5cm 0},clip,width=0.31\textwidth]{nuplots/dens-plt-b300p512nu0_4hyb850t1.pdf}
\includegraphics[trim={1cm 0 1.5cm 0},clip, width=0.31\textwidth]{nuplots/dens-plt-b300p512nu0_4hyb850t2.pdf}
\includegraphics[trim={1.5cm 0 0.5cm 0},clip, width=0.322\textwidth]{nuplots/dens-plt-b300p512nu0_4p1024t2.pdf}
...
\includegraphics[width=0.45\textwidth]{nuplots/pks_rel-0_66670.pdf}
  \caption{Ratio of matter power spectrum with massive neutrinos ($M_\nu = 0.4$ eV) to matter power spectrum with massless neutrinos. Figure shows hybrid (\texttt{HYBRID}), linear response (\texttt{LINRESP}), and particle (\texttt{PARTICLE-1024}) methods at $z=0$ (left) and $z=0.5$ (right). All three simulation methods agree with $\sim 0.1$\%, \yah{For the $P(k)$ itself, not the suppression, I presume? To clarify.} \spb{It is the same!} in agreement with AHB13. Achieving convergence for the particle neutrino method required a large number of neutrino particles, and thus almost an order of magnitude more CPU time, as discussed in Section~\ref{sec:matterpower}.}
