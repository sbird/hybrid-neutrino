...
Neutrino are the lightest standard model particles, and neutrino oscillation experiments have shown that the sum of the neutrino masses is $M_\nu > 0.06$ eV \citep{Becker-Szendy_1992, Fukuda_1998}.
However, measuring the neutrino mass in the laboratory is challenging due to the large difference in mass scales between neutrinos and other standard model particles \citep{Katrin}.
Instead, the neutrino mass can be measured through its effect on the growth of cosmic structure. Massive neutrinos behave as light thermal relics, suppressing clustering below their thermal
free-streaming length: for reviews of this effect see \cite{Lesgourgues_2006, Wong_2011, Lesgourgues_2014}. By measuring the clustering of matter and matter tracers in the Universe, cosmologists aim to detect this suppression and
thus measure the neutrino mass. So far several experiments have placed upper limits on the sum of the neutrino mass. Currently the most stringent constraints come from the \Lya~forest \cite{Palanque_2015}
in combination with the Planck cosmic microwave background experiment. Planck alone constrains $M_\nu < 0.7$ eV \citep{Planck_2015}, while the addition of the \Lya~forest data constrains $M_\nu < 0.12$ eV.
Various other current and future structure formation probes have also constrained the neutrino mass. In particular, the Dark Energy Survey and Hyper Suprime Cam weak lensing experiments have
constrained $M_\nu < ???$ eV \citep{DES} and $M_\nu < ???$ eV \citep{HSC}. The most recent galaxy power spectrum measurements from the Sloan Digital Sky Survey prefer a non-zero neutrino mass of $M_\nu = 0.3$ eV
at moderate significance \citep{Beutler_2014}. Near-future lensing experiments will have the statistical power to measure the neutrino mass even if it is close to $0.06$ eV, the minimum required
by oscillation experiments \citep{Abazajian_2006}.
...
of structure growth. Forthcoming surveys will derive their statistical power from small scales where structure growth has become non-linear \citep{someone}, and thus the only way to achieve this is
