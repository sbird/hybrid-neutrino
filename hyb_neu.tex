...
We show a halo mass function from \cite{Watson_2013} modified to match the massive neutrino cosmology by setting $\sigma_8 = 0.7372$, and $\Omega_M = \Omega_\mathrm{CDM} + \Omega_b$ (ie, excluding the neutrino component), as recommended by \cite{Castorina_2014}. Our box size is unfortunately too small to investigate the dominant theoretically expected effect of massive neutrinos, an increased suppression in large halos. On small scales our simulations are discrepant with the halo mass function at the $20\%$ level. Interestingly, massive neutrinos appear to increase the number of $10^{12} M_\odot$ halos. While it is possible that this is due to the change neutrinos induce in the growth rate, it is limited to $z<1$ and so may also be an artifact of our small box.
Note that if the \texttt{CDM} and \texttt{HYBRID} halo mass functions were normalised to the same value at $10^{12} M_\odot$, the shape of the mass function would match that predicted theoretically.
We defer a more detailed analysis with more sophisticated halo finders and larger boxes to future work.
