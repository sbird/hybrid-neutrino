...
\caption{Halo mass functions at $z=0$ for our \texttt{PARTICLE-1024}, \texttt{LINRESP} and \texttt{HYBRID} simulations with $M_\nu = 0.4$ eV and the \texttt{CDM} simulation with $M_\nu = 0$. We show for comparison a halo mass function from \protect\cite{Watson_2013} with $\sigma_8$ matching the massive neutrino simulations, as described in the text. All curves are normalized by a halo mass function with $\sigma_8$ corresponding to from the \texttt{CDM} simulation.}
  \label{fig:halomass}
\end{figure}

Figure~\ref{fig:halomass} shows the halo mass functions for our particle, linear response and hybrid simulations. In order to easily compare the different simulations, halos are composed only of CDM particles; neutrino particles are excluded. All halo mass functions have been normalized by the halo mass function predicted by \cite{Watson_2013} using cosmological parameters corresponding to our CDM simulation; $\sigma_8 = 0.8375$. We use the \cite{Watson_2013} halo mass function because our simulations use the same halo finder as their calibration simulations: a friends-of-friends mass function with a linking length of $0.2$.

We show a halo mass function from \cite{Watson_2013} modified to match the massive neutrino cosmology by setting $\sigma_8 = 0.7372$, and $\Omega_M = \Omega_\mathrm{CDM} + \Omega_b$ (ie, excluding the neutrino component), as recommended by \cite{Castorina_2014}. On small scales our simulations are discrepant with the halo mass function at the $20\%$ level. Interestingly, massive neutrinos appear to increase the number of $10^{12} M_\odot$ halos. \cite{Liu_2017} examined the halo mass function with neutrino simulations using the LRA, larger boxes and the \texttt{ROCKSTAR} halo finder \citep{Behroozi_2013}, showing good consistency with the mass function of \citep{Tinker_2008}. While it is possible that the discrepancies with theory we see are due to our higher neutrino mass, it is more likely to be due to a quirk of FoF halo finders or our relatively small box.

Most importantly, however, Figure~\ref{fig:halomass} shows that the simulated mass functions are nearly the same in each neutrino simulation method. Small differences are visible in the largest halo mass bins, where the total number of halos in the box is small and small changes in mass distribution can move a halo to an adjacent bin. There is also a $5\%$ scatter between individual mass bins, due to Poisson noise in the halo catalogue from our relatively small simulation box. This scatter is the same for all our simulations as we have used the same initial particle realization in each case. All our methods of simulating neutrinos thus produce similar results for strongly non-linear structures.
