...
\begin{abstract}
\yah{Title suggestion: Efficient and accurate hybrid method for simulating nonlinear neutrino structure }
...
However, measuring the neutrino mass in the laboratory is challenging due to the large difference in mass scales between neutrinos and other standard model particles \cite[although see][]{Wolf_2010}. \yah{Is the small mass the reason why it is challenging, or just because they interact so little? Want to make sure we don't describe particle-physics experiments challenges inaccurately.}
...
\yah{Moved the paragraph on caveats of particle method to the introduction.} The standard approach to include massive neutrinos in structure-formation simulation has been to use the $N$-body method, which consists in solving for the evolution of discrete phase-space chunks or ``particles", akin to those used for CDM, but with a %lower particle mass and a
...
The linear-response approximation (herafter, LRA), sometimes referred to as the \cite{Gilbert_1966} approximation, consists in linearizing the collisionles Boltzmann equation in the gravitational interaction. Formally, it is the first order of an expansion in $\phi/v^2$, where $v^2$ is the variance of the velocity distribution; in particular, it is only well defined for \emph{hot} species, and is not adapted to standard cold dark matter \citep{YAH_15}. It is equivalent to the Born approximation, i.e.~to assuming nearly-straight-line trajectories, with small deflections of order $\phi/v^2$. The LRA was first applied to cosmological neutrinos by \cite{Bond_Szalay_1983} and \cite{Brandenberger_1987}, and by several authors since then \citep{Singh_Ma_2003, Ringwald_Wong_2004}. In AHB13, the LRA was applied to the entire neutrino distribution, in order to model their clustering in the non-linear regime of structure formation. The goal of this section is to study in more detail the regime of validity of this approximation.


\subsection{General equations}


%AHB13, motivated by numerical difficulties with particle simulations, devised an analytic, linear response approximation for modelling massive neutrinos.


We start by briefly reviewing the LRA for a general collisionless species with phase-space density $f$. We work in the deep sub-horizon limit, in the conformal Newtonian gauge \citep{Ma_1995}. We denote by $s$ the ``superconformal" or ``Newtonian" time $(ds \equiv dt/a^2$, where $a$ is the scale factor), and overdots denote derivatives with respect to $s$. Comoving scales are denoted by $\bs{x}$ and $\bs{u} \equiv \dot{\bs{x}}$ denotes the rescaled peculiar velocity of a massive particle. We normalize the phase-space density such that the overdensity is given by
\beq
...
&+& i \bs{k} \cdot \nabla_{\bs{u}} f_0 \int_{s_i}^s d s' \rme^{- i \bs{k} \cdot \bs{u} (s - s')} a'^2 \phi(s', \bs{k}).~~~
\earr
...
\earr
where the first piece corresponds to the free propagation of initial perturbations from $s_i$ to $s$, and the kernel $\mathcal{I}$ is the Fourier transform of the unperturbed phase-space density \citep{Brandenberger_1987, Bertschinger_Watts_1988}:
\barr
\mathcal{I}(\bs{k}\Delta s ) % &\equiv& - \frac{i}{k^2 \Delta s} \int d^3 u  ~\bs{k} \cdot \nabla_{\bs{u}} f_0 ~\rme^{- i \bs{u} \cdot \bs{k} \Delta s} \nonumber\\
%&=& - \frac{1}{k \Delta s} \int d^3 u  \frac{d f_0}{du} j_1(u k \Delta s) \nonumber\\
= \int d^3 u ~ f_0(\bs{u})~\rme^{- i \bs{u} \cdot \bs{k} \Delta s}. \label{eq:I(k)}
\earr
%where in the last line we have integrated by parts.
If, moreover, $f_0(\bs{u}) = f_0(u)$ is isotropic, the latter expression simplifies to
\beq
\mathcal{I}(k\Delta s ) = \int 4 \pi u^2 d u ~ f_0(u)~ j_0(u k \Delta s), \label{eq:I(k)-iso}
\eeq
...
\begin{equation}
 ...
 \label{eq:phases}
\end{equation}
in the integral, and justify this approximation now.

We denote by $u_0$ the characteristic velocity of $f_0$, defined as
\beq
u_0 \equiv \left(\int 4 \pi du~ f_0(u)\right)^{-1/2},
\eeq
...
\eeq
regardless of the detailed evolution of $\delta_M$. Therefore, provided the non-linear scale is smaller than the free-streaming scale, we may safely use Eq.~\eqref{eq:phases} on all scales: it is accurate on linear scales, and does not affect the asymptotic value \eqref{eq:delta-small} of the perturbation on scales smaller than the free-streaming scale.
...
\eeq
At $z = 0$, the free-streaming scale is larger than the non-linear scale for $u_0 \gtrsim 300$ km/s. For $z = 1$, this condition is satisfied for $u_0 \gtrsim 100$ km/s. Provided we only use the LRA for velocities larger than these thresholds, we may safely use the approximation \eqref{eq:phases} inside of Eq.~\eqref{eq:delta-phi}. Let us note that it would have been equally justified to use the ratio of linear growth rates instead of the ratio of $P_{M}^{1/2}$ in Eq.~\eqref{eq:phases}. We choose the latter as it is directly computed from our code and does not require an additional integration with a linear Boltzmann code. In practice, we store the one-dimensional matter power spectrum at even intervals $\Delta a = 0.01$, and then interpolate it when required\footnote{The code actually provides the CDM power spectrum. We estimate the matter power spectrum by adding the neutrino power from the previous timestep. It would be possible to iterate this procedure, but in practice it is always immediately converged to a high degree of accuracy, see Appendix B of AHB13.}.
...
%This approximation is a priori only accurate on linear scales. However, as argued in AHB13, the non-linear scale is smaller than the free-streaming scale, which implies that on non-linear scales the kernel $\mathcal{I}$ suppresses early-time contributions to the integral. Thus all that is needed is the present-day phase information and, as a consequence, this approximation ought to be reasonably accurate on all scales.


We can confirm this expectation using a particle neutrino simulation. Eq.~\eqref{eq:phases} implies that the cross-correlation between neutrinos and CDM is unity. Figure \ref{fig:cross-corr} shows the cross-correlation coefficient between neutrino particles and CDM in a particle simulation and in our new hybrid method, demonstrating that it is indeed close to unity as long as the neutrino power spectrum is not dominated by particle shot noise. This validates our use of Eq.~\eqref{eq:phases}. \yah{I am not sure this plot is the best way to validate this approximation (but perhaps it can serve another purpose). First, Eq.~\eqref{eq:phases} \emph{implies}, but is \emph{not equivalent} to a unit cross-correlation coefficient. Therefore, showing that this coefficient is unity is a consistency check at best, but does not fully validate the approximation. Second, the approximation only applies to LRA neutrinos, \emph{not} to the slow neutrinos, so showing their correlation coefficient (green line) is not relevant for this specific point (unless you wanted to make another point?). Since the red curve contains the slow neutrinos as well, which we do not expect to have a unit correlation, it is not unexpected that it falls below unity (and it is hard to quantify by how much we expect it to do so). What would have been a good check is to show the cross-correlation of fast neutrinos only (from the particle sims), but this is probably not easily extracted. All in all, I think that the agreement with Banerjee et al. is sufficient to validate both layers of approximation, linear response \emph{and} phase approximation.}
...
\includegraphics[width=0.45\textwidth]{nuplots/corr_coeff-1.pdf}
  ...
  The cross-correlation is close to $> 0.8$ when shot noise is small, justifying the approximation that neutrinos and CDM are completely correlated. \yah{See comments in the main text, I am not sure this plot makes the point it is intended to make. }
  }
...
In AHB13, the linear-response approximation was used for the \emph{entire} phase-space of neutrinos. Yet, this approximation should eventually break down below some critical velocity $u_{\rm crit}$, as the behaviour of neutrinos approaches that of the CDM. The goal of this section is to estimate $u_{\rm crit}$. Whenever required, we use the non-linear matter power spectrum estimated by \textsc{class} \citep{Lesgourgues_11, Blas_11} with the \textsc{halofit} approximation \citep{Smith_2003}.

\subsubsection{Single velocity shell}

We first consider an isotropic unperturbed distribution that is peaked at a narrow velocity shell, i.e.~
\beq
f_0(u) = \frac1{4 \pi u_0^2} \delta_{\rm D}(u- u_0).
\eeq
Inserting this distribution into Eqs.~\eqref{eq:delta-phi}--\eqref{eq:I(k)} with the approximation \eqref{eq:phases}, and neglecting the initial condition piece we obtain the overdensity
\barr
\delta_{u_0}(s, \bs{k}) &\approx& \int_{s_i}^s ds' a' \sin[k u_0 (s - s')]
\left(\frac{P_M(s', k)}{P_M(s, k)}\right)^{1/2} \nonumber\\
&&\times \frac32 \frac{H_0^2 \Omega_M}{k u_0} \delta_M(s, \bs{k}),
\earr
implying a power spectrum
\barr
P_{u_0}(s, k)&\approx& \left(\frac32 \frac{H_0^2 \Omega_M}{k u_0}\right)^2\nonumber\\
&\times& \left[\int_{s_i}^s ds' a' \sin[k u_0 (s - s')] \sqrt{P_M(s', k)} \right]^2. ~~~~~
\earr
...
We notice in Fig.~\ref{fig:simvshell} that even for the lowest velocity shell, for which the LRA underestimates the correct clustering, the power per logarithmic $k$-interval remains significantly below unity. The power per shell is therefore not a good indicator of non-linear clustering, and below we study the clustering of individual velocity \emph{bins}, i.e.~for a non-isotropic unperturbed phase-space distribution of the form
\beq
f_0(\bs{u}) = \delta_{\rm D}(\bs{u}- \bs{u}_0).
\eeq
...
\earr
The power spectrum of $\delta_{\bs{u}_0}$ is then
\barr
P_{\bs{u}_0}(s, \bs{k}) =\left(\frac32 H_0^2 \Omega_M\right)^2 \iint_{s_i}^s ds' ds'' (s - s') a' (s - s'') a'' \nonumber\\
\rme^{i \bs{u}_0 \cdot \bs{k} (s'' - s')} P_M(s', s'', k), \label{eq:P_u0}
\earr
where the unequal-time power spectrum is
\beq
\langle \delta_M^*(s',\bs{k}')\delta_M(s'',\bs{k})\rangle \equiv (2 \pi)^3 P_M(s', s'', k) \delta_{\rm D}(\bs{k}' - \bs{k}).
\eeq
We are interested in the real-space variance, obtained by integrating over wavenumbers. In particular, the variance per $k$-interval is obtained by averaging $P_{\bs{u}_0}(\bs{k})$ over the orientations of $\bs{k}$:
\barr
\langle P_{\bs{u}_0}(s, \bs{k})\rangle_{\hat{k}} = \left(\frac32 H_0^2 \Omega_M\right)^2 \iint_{s_i}^s ds' ds'' (s - s') a' (s - s'') a'' \nonumber\\
j_0[u_0 k (s'' - s')]~ P_M(s', s'', k). \label{eq:P_u0-av}
\earr
...
On linear scales
\beq
P_M(s', s'', k) \approx \sqrt{P_M(s', k)P_M(s'', k)}. \label{eq:P-unequal}
\eeq
On scales smaller than the free-streaming scale, we may approximate $j_0[u_0 k (s'' - s')] \approx \frac{\pi}{2 u_0 k} \delta_{\rm D}(s'' - s')$, implying
\barr
...
\earr
Therefore, provided the free-streaming scale is larger than the non-linear scale, we may use Eq.~\eqref{eq:P-unequal} in all cases to compute Eq.~\eqref{eq:P_u0-av}.

We show in Fig.~\ref{fig:halofitvshell} the resulting power per logarithmic $k$-interval, $\Delta^2(k, u_0) = k^3/(2\pi^2)\langle P_{\bs{u}_0}(s, \bs{k})\rangle_{\hat{k}} $, at $z = 0$ and 1. We see that at $z = 1$, $\Delta^2(k, u_0)$ is less than unity on all scales for $u_0 \gtrsim 100$ km/s. On the other hand, at redshift 0, only neutrinos faster than $\sim 800$ km/s are statistically unclustered. This explains the deviation of LRA from the numerical results of B18 found in the previous section for the lowest velocity shell.


\begin{figure*}
\includegraphics[width=0.47\textwidth]{nuplots/lin_resp_z1.pdf}
\includegraphics[width=0.47\textwidth]{nuplots/lin_resp_z0.pdf}
\caption{Angle-averaged power per logarithmic $k$-interval, in the linear-response approximation, for an unperturbed phase-space density $f_0(\bs{u}) = \delta_{\rm D}(\bs{u}- \bs{u}_0)$. The curves are labeled by the unperturbed velocity $u_0$. At redshift 1 (left panel), we see that particles with unperturbed velocities as low as $\sim 100$ km/s do not strongly cluster on all scales. At redshift 0 (right panel), particles with unperturbed velocities $u_0 \gtrsim 800$ km/s do not strongly cluster on all scales.  \spb{Change velocities used to 0, 100, 400, 800, 1000, make labels bigger, shrink range, add redshift.} \yah{I chose the velocities so they are spaced by even factors, which gives a nice regular decrease (the power goes as $1/u_0$ for large $k$), and the $k$-range to match that of our simulation plots, and wide enough to show that all curves converge at low $k$. What $k$-range did you have in mind?}}
...
The basic idea of the hybrid method we propose is to split neutrinos into a ``slow" and a ``fast" component, depending on their \emph{unperturbed} velocity relative to some critical velocity $v_{\rm crit}$, as illustrated in Fig.~\ref{fig:fddistribution}. %Because at linear order velocity shells do not mix, neutrinos which are initially labelled as fast or slow remain within these categories throughout their evolution.
Because the split is done in terms of unperturbed velocities, the ``slow" and ``fast" labels can be thought of ``Lagrangian'' labels. As a consequence, both the slow and fast components independently satisfy the collisionless Boltzmann equation\footnote{If the split between ``slow" and ``fast" had been made instead in terms of the perturbed or ``Eulerian" velocity, one would have to solve for two coupled collisional Boltzmann equations to account for the flux of particles across the boundary.}. \yah{I emphasize that velocity bins do mix even at linear order. But they don't if they are Lagrangian labels, even at arbitrary non-linear order!} We are free to solve these two equations with different techniques: the LRA for fast particles and the $N$-body method for slow particles. This allows us to focus the computationally expensive $N$-body technique only for the fraction of phase-space where it is truly needed.
...
\includegraphics[width=0.45\textwidth]{nuplots/fermidirac.pdf}
  ...
  The grey shaded region shows the neutrino density followed by particles in our hybrid method. \yah{redo shaded area with 850 since it is our new default $v_{\rm crit}$.}
  }
...
\eeq
Therefore, by treating \emph{all} neutrinos with the linear-response approximation at $z \geq 1$, we underestimate the clustering of at most $\sim 0.1\%$ of the neutrinos, given current upper bounds to their masses. This small error is clearly below the precision of $N$-body codes and the theoretical uncertainty of baryonic physics, so we deem it safe to use the method of AHB13 (the LRA applied to \emph{all} neutrinos) for $z \geq 1$, i.e.~to set $z_\nu = 1$.
...
\subsection{Linear-response kernel for truncated Fermi-Dirac distribution} \label{sec:kernel}

Once the slow neutrinos are followed as particles, the LRA is applied only to fast neutrinos, whose unperturbed density is the truncated (high-pass filtered) Fermi-Dirac distribution:
\begin{equation}
...
\end{align}
We have checked that this expansion is converged to within an absolute error of $< 10^{-4}$ for $n_\mathrm{max} = 20$.
...
The numerical overhead is thus substantially reduced for the hybrid method over pure particle neutrino simulations. This is because the neutrino particles have a slower average velocity in the hybrid method and thus allow for longer timesteps.
...
\yah{Reorganized as follows:  (ii) general things common to all sims, (iii) general description of three simulation methods (iv) particle method convergence [see text for comments] (v) tests of the hybrid method}
...
\texttt{PARTICLE-MINNU}    &     0.06 (NH)      & 1024 \yah{??}         & - &  -  \\
\texttt{LINRESP}   &     0.4            & 0         & - &   - \\
...
\end{tabular}
\end{center}
\caption{\yah{I suggest the above more explicit and systematic naming convention, this way the reader can easily guess the parameters without having to always look them up in the table.} Table of simulations performed. All simulations have a box of $300$ Mpc/h
and $512^3$ CDM particles. NH denotes a normal hierarchy for the neutrino masses.
For $M_\nu = 0.4$ eV, the fraction of neutrinos slower than critical velocities of $750$, $850$, $1000$, $5000$ km/s is $0.276$, $0.346$, $0.451$ and $1$ respectively.}
\label{tab:simulations}
\end{table}
...
\includegraphics[trim={1.5cm 0 0.5cm 0},clip, width=0.322\textwidth]{nuplots/dens-plt-b300p512nu0_4pt2.pdf}
  ...
  The clustering of the hybrid particles is intermediate between that of the cold dark matter and the particle neutrinos. Structures have the same positions in all three panels. \yah{Suggestions: $(i)$ drop the axes so that the plots can be made bigger, and simply say in the caption that the boxes are 300 by 300 Mpc [and what depth btw?]. $(ii)$ if possible, use a different scale for neutrinos, because the rightmost plot is barely visible. This will illustrate the shot noise more clearly}
  }
...
\includegraphics[width=0.45\textwidth]{nuplots/pks_rel-0_66670.pdf}
  \caption{Ratio of matter power spectrum with massive neutrinos ($M_\nu = 0.4$ eV) to matter power spectrum with massless neutrinos. Figure shows hybrid (\texttt{HYBRID}), linear response (\texttt{LINRESP}), and particle (2xPARTICLE) \yah{\texttt{PARTICLE-1024}} methods at $z=0$ (left) and $z=0.5$ (right). All three simulation methods agree with $\sim 0.1$\%, \yah{For the $P(k)$ itself, not the suppression, I presume? To clarify.} in agreement with AHB13. Achieving convergence for the particle neutrino method required a large number of neutrino particles, and thus almost an order of magnitude more CPU time, as discussed in Section~\ref{sec:matterpower}.}
  \label{fig:matter_power}
\end{figure*}

\begin{figure}
\includegraphics[width=0.45\textwidth]{nuplots/pks_lowmass-10.pdf}
\caption{Ratio of the matter power spectrum with the minimal neutrino mass ($M_\nu = 0.06$ eV) to the matter power spectrum with massless neutrinos at $z=0$. The linear response method (MINNU) and particle method (MINNU-PART) are shown. At these low masses, the particle method is affected by shot noise. \yah{\texttt{PARTICLE-MINNU, HYBRID-MINNU}}}
...
\caption{Halo mass functions at $z=0.2$ for our particle, linear response and hybrid simulations with $M_\nu = 0.4$ eV, as well as the mass function model from \protect\cite{Watson_2013} with parameters matching the massive neutrino model. We normalise by the mass function model with parameters corresponding to $M_\nu = 0$. \yah{\texttt{PARTICLE-1024}. This plot is strange as the ratio goes to zero (!), which kind of makes it suspicious...}.}
  \label{fig:halomass}
\end{figure}
...
\begin{figure}
  \includegraphics[width=0.45\textwidth]{nuplots/pks_nu_ckrel2-1.pdf}
    \caption{Ratios of neutrino power spectra to the results of the default hybrid simulation. Shown are the default (\texttt{HYBRID}), neutrino particle simulations with $512^3$ neutrino particles (\texttt{PARTICLE}) and $1024^3$ neutrino particles (2xPARTICLE) \yah{\texttt{PARTICLE-1024}}, and a hybrid simulation where all neutrino mass density is in particles after the switch-on time (HYBALL) \yah{\texttt{HYBRID-v5000}}. Power spectra have been smoothed with an 11-pt rolling average to reduce scatter and shot noise has been subtracted.}
  \label{fig:hybparticle}
\end{figure}

\begin{figure}
  \includegraphics[width=0.45\textwidth]{nuplots/hyb_test_YAH.pdf}
  \caption{\yah{Suggestion of hyper-explicit legend (original plot still there!!) Alternate suggestion is to use the explicit naming convention. I had to go back and forth with the table/caption to figure out what is what.} Neutrino power spectra from various hybrid simulations compared to the results of the default hybrid simulation (\texttt{HYBRID}). \texttt{HYBRID} has a cut-off velocity of $850$ km/s, a neutrino switch-on time of $z=1$ and $512^3$ neutrino particles. Other simulations shown have neutrino cut-off velocities of $1000$ km/s (VCRIT) \yah{\texttt{HYBRID-v1000}} and $750$ km/s (VCRITLO) \yah{\texttt{HYBRID-v750}}, a switch-on time of $z=4$ (NUTIME) \yah{\texttt{HYBRID-z4}} and $256^3$ neutrino particles (HYBSING) \yah{\texttt{HYBRID-256}}. Power spectra have been smoothed with an 11-pt rolling average to reduce scatter and shot noise has been subtracted.}
  \label{fig:vcrit}
\end{figure}

Figure~\ref{fig:neutrino_power} shows the neutrino power spectrum at $z=0$ and $z=0.5$, demonstrating each of the neutrino simulation methods, as well as the linear theory power spectrum from CAMB. For the hybrid simulation we have computed the total neutrino power spectrum (assuming both neutrino components are completely correlated) as the weighted sum of the power spectrum of the fast and slow components $P^{1/2}_\nu = f_\mathrm{fast} P^{1/2}_\mathrm{fast} + f_\mathrm{slow} P^{1/2}_\mathrm{slow}$, where $f_\mathrm{fast} + f_\mathrm{slow} = 1$. \yah{This is clearly not correct on non-linear scales (see Figure 1). Is it not possible to compute explicitly the total neutrino density field and take its power spectrum? Are all the ratios of $P_\nu$ computed this way? If so, could this account for part of the deviations seen in neutrino power comparison? From Figure 1, it seems that it could be a $\sim$5-10\% effect.}  Figure~\ref{fig:neutrino_power_split} shows the neutrino power spectra for each individual component, demonstrating that the slow neutrinos do indeed cluster substantially more.

Neutrino particle shot noise has been subtracted from both the particle and hybrid simulation. The shot noise for the hybrid simulation is $P_\mathrm{shot} = 0.346\times (300 /512)^3 $ (Mpc/$h)^3$, where the factor of $0.346$ is due to the reduced matter density in particle neutrinos \yah{should this be squared?}. The particle simulation has $P_\mathrm{shot} = (300 /1024)^3$ (Mpc/$h)^3$. In both simulations, the neutrino power is recoverable even two orders of magnitude below the shot noise level, indicating that there is little structure formation arising purely from neutrino shot noise. \yah{I don't think this is really a worry because shot noise should be mostly completely uncorrelated from one timestep to the next. But there may be a way to understand the residual noise on small scales, will think about it.}
...
& \yah{3 masses but no hierarchy, ``for full generality"?}\\
...
%\yah{I don't understand, why not just take the time derivative of CAMB's output??} \spb{At the time, this was hard to implement (and doesn't make a difference). It is easier to implement now though, so I will do it while you read this version.}

Our initial conditions are generated using the CAMB total matter transfer
function \yah{We are talking about CDM here, so I hope by ``total" you mean CDM + baryon only?} at $z=99$. An alternative is to generate initial conditions
...
\yah{This needs to be clarified. First, I presume you mean the sub-horizon growth function (otherwise, the growth function is, by definition, the ratio of the output of CAMB at different redshifts). Second, while the box size is smaller than the neutrino free-streaming scale at the initial reshift, it is no longer the case at $z = 0$. So the rescaling would have to be scale-dependent, (even if only sub-horizon). My understanding is that this re-scaling business is done to compensate for not properly treating super-horizon scales, so using an incorrect scale-independent rescaling means that we now make an error on small scales instead (as can be seen in the figures). Moreover, the rescaling is done to match a particular final redshift, so it is expected that the $z = 9$ will not match perfectly.}
...
\yah{I thought the point that radiation needs to be included in the Hubble rate had already been made elsewhere. If so, I don't think it deserves being re-stated here.}
