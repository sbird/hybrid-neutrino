...
However, \citep{AHB} also found that this method did not reproduce the power spectrum of the neutrinos at low redshift ($z < 0.5$), despite the fact that the linear neutrino perturbation remains perturbative.
...
Our model is a hybrid approach, combining our earlier analytic framework for low neutrino masses and early times with a neutrino particle implementation for
...
even initially, and so mitigate the numerical problems that plague purely particle simulations. Our new method allows, for the first time, a single simulation code to produce a well-converged neutrino simulation, at any neutrino mass, which includes both early-time relativistic effects and late-time non-linear growth in the neutrino sector. It could be used, for example, to make predictions for dedicated cosmic neutrino background experiments \citep{Ptolemy}. We describe it in more detail in Section~\ref{sec:hybrid}.

The earlier implementation in \cite{AHB} was somewhat difficult to use, being tied to a non-public version of the simulation code Gadget-3 \cite{Springel_2005}.
Here we provide a public implementation of this simulation method, which is fully integrated into the public cosmological simulation
code MP-Gadget \cite{Feng_2016}, available as a patch to Gadget-2, and designed to be portable to many similar structure simulation codes.
The Gadget-2 port in particular is well-tested and has already been used to run a large suite of simulations \citep{Liu_2017}.
