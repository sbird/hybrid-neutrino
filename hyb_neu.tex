...
A tree gravity solver uses nodes, which resemble particles of particles. For reasons of efficiency, Gadget keeps these particles of particles around between timesteps, and moves them according to the average velocity of the particles within them. This works poorly when the particles in the node all have large random thermal velocities! So now we do not keep the particles around between timesteps anymore. This is a Gadget-3 specific bug.

Gadget allows particles to have different timesteps and uses a kick-drift-kick momentum transfer scheme. When it wanted to output a snapshot, it would drift all particles to the desired time and write them. Because drifting does not include a kick, this meant that the neutrino momenta were incorrect. So now I build the timesteps around when we want outputs. This *was* documented, so isn't really a bug, but was unhelpful. 

\subsection{Analytic Neutrinos}
\label{sec:analytic}

In this section we describe the semi-analytic linear response method for treating neutrinos from \cite{YAHSB}.
...
almost as expected from linear perturbation theory. Following \cite{YAHSB}, we define $\mathcal{I}$ to 
...
The neutrino power-spectrum is then given by (eq. (63) of \cite{YAHSB})
\begin{align}
...
The main improvement of this method over previous perturbation-based neutrino codes \citep{Brandbyge:2009}
is our use of the full non-linear CDM potential in Eq.~\ref{eq:P-final}, which allows us 
to achieve accurate results for any level of CDM clustering, as long as neutrinos themselves are still
clustering linearly.

As shown in \cite{YAHSB}, this method computes the total matter power spectrum to better 
